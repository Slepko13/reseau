...
const ProfileInfo: React.FC<ProfileInfoPropsType> = (props) => {
      let [button, toggleButton] = useState(true);
      ...
      const onSubmit = (values: ProfileType) => {
            props.changeProfileData(values).then(
                  () => { setEditMode(false); }
            )
      }
...
const ProfileData: React.FC<ProfileDataPropsType> = ({ profile, isOwner, activateEditMode }) => {
      ...
      return (
            <div className="ProfileData">
                  {!isOwner ? <div><button onClick={activateEditMode}>Edit</button></div> : undefined}
                  <div>My name: {profile.fullName}</div>
                  <div>Looking for a job :  {profile.lookingForAJob ? "Yes" : "No"}</div>
                  {
                        profile.lookingForAJob ?
                              <div>My professional skills :{profile.lookingForAJobDescription}</div> : undefined
                  }
